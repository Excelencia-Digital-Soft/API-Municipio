...
{
    public class PagoRepository : IPagoRepository// : BaseRepositoryReceta<Receta>, IRecetaRepository
    {
        private MunicipalidadContext _context;
        public PagoRepository(MunicipalidadContext context)
        {
            _context = context; 

        }
        public async Task<List<ListadoGeneralDTO>> ListadoPagos(int pId_Municipio, DateTime pFechaDesde, DateTime pFechaHasta)
        {
            var fd = pFechaDesde.ToString("yyyyMMdd");
            var fh = pFechaHasta.ToString("yyyyMMdd");
            using (var context = new MunicipalidadContext())
            {
                return await context.Database
                    .SqlQueryRaw<ListadoGeneralDTO>("EXEC ListadoFacturas @pId_Municipio, @pFechaDesde, @pFechaHasta",
                        new SqlParameter("@pId_Municipio", pId_Municipio),
                        new SqlParameter("@pFechaDesde", fd),
                        new SqlParameter("@pFechaHasta", fh))
                    .ToListAsync();
            }
